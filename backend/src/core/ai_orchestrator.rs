...
use crate::models::ai_action::{
    MultiStepRequest, MultiStepResponse, MultiStepSession, SessionStatus, StepInfo, StepRequest,
    StepStatus,
};
...
impl AIOrchestrator {
    ...
    pub async fn process_multi_step(
        ...
        if let Some(depends_on) = &request.current_step.depends_on {
            let dependency_completed = session
                .steps
                .iter()
                .any(|s| s.step_id == *depends_on && s.status == StepStatus::Completed);
        ...
        // 执行当前步骤
        let step_result = Self::execute_step(pool, &request.current_step, user_id, user_name).await;
        ...
        // 生成下一步建议
        let next_step_suggestions =
            Self::generate_next_suggestions(&session, &request.current_step, &current_step_result);
