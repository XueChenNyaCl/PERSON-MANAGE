...
async fn main() -> Result<(), anyhow::Error> {
    ...
    // 检查admin用户是否存在
    match sqlx::query(
        "SELECT id, username, name, role, is_active FROM persons WHERE username = 'admin'",
    )
    .fetch_optional(&pool)
    .await
    {
        ...
        Ok(None) => {
            ...
            // 列出所有用户
            match sqlx::query(
                "SELECT username, name, role FROM persons WHERE username IS NOT NULL LIMIT 10",
            )
            .fetch_all(&pool)
            .await
            {
        ...
        .await
    {
        Ok(rows) => {
            ...
            for row in rows {
                ...
                println!(
                    "      - ID: {}, Name: {}, Type: {}, Username: {:?}, Role: {:?}",
                    id, name, person_type, username, role
                );
            }
        }
