...
impl NameResolver {
    /// 解析人员名称，返回人员ID
    /// 如果有多个人员同名，返回候选项
    pub async fn resolve_person(pool: &PgPool, name: &str) -> Result<ResolutionResult, AppError> {
        ...
        if persons.is_empty() {
            return Ok(ResolutionResult::NotFound(format!(
                "未找到人员: '{}'（可用姓名/学号/工号/用户名/手机号）",
                keyword
            )));
        }
    ...
    /// 解析小组名称，返回小组ID
    pub async fn resolve_group(pool: &PgPool, name: &str) -> Result<ResolutionResult, AppError> {
        // 首先尝试直接作为UUID解析
    ...
    /// 解析班级名称，返回班级ID
    pub async fn resolve_class(pool: &PgPool, name: &str) -> Result<ResolutionResult, AppError> {
        // 首先尝试直接作为UUID解析
        if let Ok(uuid) = Uuid::parse_str(name) {
            let exists: bool =
...
impl ParamAutoCompleter {
    ...
    pub fn complete_date(date_str: &str) -> Option<String> {
        ...
        match date_str.trim() {
            "今天" | "今日" | "today" | "now" => Some(today.format("%Y-%m-%d").to_string()),
            "明天" | "明日" | "tomorrow" => Some(
                (today + chrono::Duration::days(1))
                    .format("%Y-%m-%d")
                    .to_string(),
            ),
            "昨天" | "昨日" | "yesterday" => Some(
                (today - chrono::Duration::days(1))
                    .format("%Y-%m-%d")
                    .to_string(),
            ),
    ...
    pub fn complete_time(time_str: &str) -> Option<String> {
        ...
        if let Some(start) = hour_start {
            let hour_end = cleaned[start..]
                .find(|c: char| !c.is_ascii_digit())
                .unwrap_or(cleaned.len() - start);
            ...
            if let Some(colon_pos) = cleaned.find(':') {
                ...
                if minute_start < cleaned.len() {
                    let minute_end = cleaned[minute_start..]
                        .find(|c: char| !c.is_ascii_digit())
                        .unwrap_or(cleaned.len() - minute_start);
...
impl AIActionExecutor {
    ...
    pub async fn execute(
        ...
        let result = match action_req.action_type.as_str() {
            "create_notice" => {
                Self::execute_create_notice(pool, &action_req.params, user_id, &user_permissions)
                    .await
            }
            "create_group" => {
            ...
            "update_group_score" => {
                Self::execute_update_group_score(
                    pool,
                    &action_req.params,
                    user_id,
                    &user_permissions,
            ...
            "create_score" => {
                ...
                if normalized.get("student_id").is_none() && normalized.get("group_id").is_some() {
                    ...
                    Self::execute_update_group_score(pool, &normalized, user_id, &user_permissions)
                        .await
                } else {
    ...
    async fn execute_create_notice(
        ...
        if Self::exceeds_char_limit(&notice_params.content, Self::MAX_NOTICE_CONTENT_LEN) {
            return Ok(Self::invalid_input_response(
                ...
                format!(
                    "公告内容长度不能超过 {} 个字符",
                    Self::MAX_NOTICE_CONTENT_LEN
                ),
            ));
        }

        // 执行创建
        let target_id = notice_params
            .target_id
            .as_ref()
    ...
    async fn execute_create_group(
            ...
            .is_some_and(|v| Self::exceeds_char_limit(v, Self::MAX_GROUP_DESCRIPTION_LEN))
        {
            return Ok(Self::invalid_input_response(
                ...
                format!(
                    "小组描述长度不能超过 {} 个字符",
                    Self::MAX_GROUP_DESCRIPTION_LEN
                ),
            ));
        }
    ...
    async fn execute_create_department(
        ...
        if !user_permissions
            .iter()
            .any(|p| p == "department.create" || p == "department.*")
        {
            return Ok(AIActionResponse {
                success: false,
    ...
    async fn execute_update_group_score(
        ...
        let action = if score_params.score_change >= 0 {
            "增加"
        } else {
            "扣除"
        };
    ...
    async fn execute_add_group_member(
        ...
        // 执行添加
        sqlx::query("INSERT INTO group_members (group_id, person_id) VALUES ($1, $2)")
            .bind(group_id)
            .bind(person_id)
            .execute(pool)
            .await
    ...
    async fn execute_remove_group_member(
        ...
        // 执行移除
        let result =
            sqlx::query("DELETE FROM group_members WHERE group_id = $1 AND person_id = $2")
                .bind(group_id)
                .bind(person_id)
                .execute(pool)
                .await
    ...
    async fn execute_create_attendance(
        ...
        // 解析人员ID（支持名称或UUID）
        let person_id =
            match NameResolver::resolve_person(pool, &attendance_params.person_id).await? {
                ResolutionResult::Single(id) => Uuid::parse_str(&id).unwrap(),
                ResolutionResult::Multiple(candidates) => {
                    return Ok(AIActionResponse {
                        success: false,
        ...
        let naive_date = match NaiveDate::parse_from_str(&date, "%Y-%m-%d") {
            ...
            Err(_) => {
                return Ok(AIActionResponse {
                    success: false,
                    message: format!(
                        "无效的日期格式: {}，请使用 YYYY-MM-DD 格式或相对日期（今天、明天等）",
                        date
                    ),
                    data: None,
                    user_permissions: user_permissions.to_vec(),
        ...
        let naive_time = if let Some(time_str) = attendance_params.time.as_ref() {
            let time_str =
                ParamAutoCompleter::complete_time(time_str).unwrap_or_else(|| time_str.clone());
            ...
            if let Ok(time) = NaiveTime::parse_from_str(&time_str, "%H:%M") {
                ...
                return Ok(AIActionResponse {
                    success: false,
                    message: format!(
                        "无效的时间格式: {}，请使用 HH:MM 格式或描述（上午8点、下午3点、现在）",
                        time_str
                    ),
                    data: None,
                    user_permissions: user_permissions.to_vec(),
        ...
        // 获取人员信息
        let person_info: (String,) = sqlx::query_as("SELECT name FROM persons WHERE id = $1")
            .bind(person_id)
            .fetch_one(pool)
            .await
    ...
    async fn execute_create_score(
        ...
        Ok(AIActionResponse {
            ...
            message: format!(
                "为 '{}' 添加个人积分 {} 分成功，原因：{}",
                person_info.0, score_params.value, score_params.reason
            ),
            data: Some(serde_json::json!({
                "id": row.id.to_string(),
                "person_name": row.person_name,
    ...
    async fn execute_create_person(
        ...
        if person_params.person_type.trim().is_empty() {
            return Ok(AIActionResponse {
                success: false,
                message: "缺少人员类型，请提供 type 或 person_type（student/teacher/parent）"
                    .to_string(),
                data: None,
                user_permissions: user_permissions.to_vec(),
                need_confirmation: false,
        ...
        match person_params.person_type.as_str() {
            "student" => {
                if person_params.student_no.is_none()
                    || person_params.student_no.as_ref().unwrap().trim().is_empty()
                {
                    return Ok(AIActionResponse {
                        success: false,
            ...
            "teacher" => {
                if person_params.employee_no.is_none()
                    || person_params
                        .employee_no
                        .as_ref()
                        .unwrap()
            ...
            _ => {
                return Ok(AIActionResponse {
                    success: false,
                    message: format!(
                        "未知的人员类型: {}，必须是 student、teacher 或 parent",
                        person_params.person_type
                    ),
                    data: None,
                    user_permissions: user_permissions.to_vec(),
                    need_confirmation: false,
        ...
        match Self::create_person_internal(pool, &person_params).await {
            Ok(person_id) => Ok(AIActionResponse {
                success: true,
                message: format!(
                    "成功创建{} '{}'",
                    match person_params.person_type.as_str() {
    ...
    async fn create_person_internal(
        ...
        let enrollment_date = params
            .enrollment_date
            .as_ref()
            .and_then(|s| chrono::NaiveDate::parse_from_str(s, "%Y-%m-%d").ok());
        let hire_date = params
            .hire_date
            .as_ref()
            .and_then(|s| chrono::NaiveDate::parse_from_str(s, "%Y-%m-%d").ok());
        ...
        let username = match params.person_type.as_str() {
            "student" => params.student_no.as_ref().cloned().unwrap_or_default(),
            "teacher" => params.employee_no.as_ref().cloned().unwrap_or_default(),
            "parent" => params
                .phone
                .clone()
        ...
        // 生成密码哈希（默认密码123456）
        let password_hash = hash_password("123456").map_err(|_| AppError::Internal)?;
    ...
    async fn execute_create_persons_batch(
        ...
        for (index, item) in items.iter().enumerate() {
            ...
            let mut person_params: CreatePersonParams =
                match serde_json::from_value(normalized_item) {
                    Ok(p) => p,
                    Err(e) => {
                        failure_count += 1;
            ...
            if person_params.person_type.trim().is_empty() {
                ...
                item_results.push(BatchItemResult {
                    ...
                    data: None,
                    error: Some(
                        "缺少人员类型，请提供 type 或 person_type（student/teacher/parent）"
                            .to_string(),
                    ),
                });
                continue;
...
pub async fn execute_action(
    ...
    // 获取用户名称
    let user_name: String = sqlx::query_scalar("SELECT name FROM persons WHERE id = $1")
        .bind(user_id)
        .fetch_one(&pool)
        .await
        .map_err(|_| AppError::NotFound)?;
...
impl AIActionExecutor {
    ...
    pub async fn execute_create_attendances_batch(
        ...
        for (index, item) in items.iter().enumerate() {
            // 解析参数
            let person_id_str = item.get("person_id").and_then(|v| v.as_str()).unwrap_or("");
            ...
            let date =
                ParamAutoCompleter::complete_date(date_str).unwrap_or_else(|| date_str.to_string());
